...
<head>
    ...
    <style>
        :root {
            ...
            --secondary:
                {{ $settings['theme_color_secondary'] ?? '#f43f5e' }}
            ;
            /* Rose */
            --text-main: #1f2937;
            --text-muted: #6b7280;
...
<body>
    ...
    <!-- 2. NAVBAR (Modern Floating) -->
    <!-- Topbar removed for cleaner look -->
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; width: 100%;">
            <a href="/" class="logo" style="display: flex; align-items: center; gap: 10px;">
                <img src="{{ $resolveAsset($settings['school_logo'] ?? '', asset('images/logo.png')) }}"
                    onerror="this.src='{{ $fallbackLogo }}'" alt="Logo" style="height: 35px; border-radius: 4px;">
                <div class="logo-text">
                    <h1 style="font-size: 1.15rem;">{{ $settings['school_name'] ?? 'SDIT Al Irsyad' }}</h1>
                </div>
            </a>
            <div style="display: flex; align-items: center; gap: 10px; margin-left: auto;">
                <ul class="nav-menu" id="navMenu">
                    ...
                    @endforeach
                    <li><a href="{{ $settings['contact_ppdb_link'] ?? '#' }}" class="nav-spmb">Daftar Sekarang</a></li>
                </ul>
