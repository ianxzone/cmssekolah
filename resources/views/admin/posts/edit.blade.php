            ...
            <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid-layout">
                    ...
                    <div>
                        ...
                        @include('admin.partials.seo-meta-box', ['model' => $post])
                    </div>
                    ...
                    <div class="sidebar-panel">
                        ...
                        @php
                            ...
                            $selectedAuthorId = old('user_id', $post->user_id ?? ($currentUser ? $currentUser->id : 1));
                        @endphp
                        <div class="form-group">
                            ...
                            @if($canChangeAuthor)
                                ...
                                <span style="font-size: 0.75rem; color: var(--text-secondary); margin-top: 0.35rem; display: block;">
                                    Sebagai <strong>{{ $currentUser->role_label ?? 'Admin' }}</strong>, Anda dapat mengganti author artikel ini.
                                </span>
                            @else
                                <div style="padding: 0.65rem 0.85rem; background: #ffffff; border: 1px solid var(--border-color); border-radius: 8px; font-size: 0.875rem; color: var(--text-primary); display: flex; align-items: center; justify-content: space-between;">
                                    <div style="display: flex; align-items: center; gap: 8px;">
                                        ...
                                        <span><strong>{{ $post->author->name ?? 'Admin' }}</strong></span>
                                    </div>
                                    <span style="font-size: 0.72rem; background: #f1f5f9; color: #475569; padding: 2px 8px; border-radius: 4px; font-weight: 600;">{{ $post->author->role_label ?? 'Penulis' }}</span>
                                </div>
                        ...
                        <div class="form-group" style="margin-top: 1.5rem;">
                            ...
                            <input type="hidden" id="featured_image_path" name="featured_image_path" value="{{ old('featured_image_path', $post->image ?? '') }}">
                            ...
                            <div id="featured-image-box" style="border: 2px dashed #cbd5e1; border-radius: 12px; padding: 1rem; background: #ffffff; text-align: center; transition: all 0.2s;">
                                <!-- Image Preview Area -->
                                <div id="image-preview-wrapper" style="position: relative; width: 100%; aspect-ratio: 16/9; background: #f3f4f6; border-radius: 8px; overflow: hidden; display: {{ !empty($post->image_url) ? 'block' : 'none' }}; margin-bottom: 0.75rem;">
                                    <img id="preview-img" src="{{ $post->image_url ?? '#' }}" alt="Featured Image Preview" style="width: 100%; height: 100%; object-fit: cover;">
                                ...
                                <div id="image-placeholder" style="padding: 1.25rem 1rem; display: {{ empty($post->image_url) ? 'block' : 'none' }};">
