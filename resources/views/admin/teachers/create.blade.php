...
@section('title', 'Tambah Tenaga Pendidik / SDM')
...
@section('content')
    <div class="panel" style="max-width: 860px; margin: 0 auto;">
        <div class="panel-header" style="display: flex; justify-content: space-between; align-items: center;">
            <h2 class="panel-title">Tambah Data SDM / Tenaga Pendidik</h2>
            ...
            <form action="{{ route('admin.teachers.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid-2">
                    <div class="form-group">
                        ...
                        <input type="text" id="name" name="name" class="form-control" value="{{ old('name') }}" placeholder="Contoh: Ustadz Budi Santoso, M.Pd." required autofocus>
                        @error('name') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="role">Jabatan / Amanah <span class="text-danger">*</span></label>
                        <input type="text" id="role" name="role" class="form-control" value="{{ old('role') }}" placeholder="Contoh: Ketua LPP / Kabid Pendidikan / Koordinator Tahfidz" required>
                ...
                <div class="grid-2">
                    <div class="form-group">
                        ...
                        <select id="unit" name="unit" class="form-control">
                            <option value="LPP" {{ old('unit') == 'LPP' ? 'selected' : '' }}>LPP (Lembaga Pendidikan & Pengajaran)</option>
                            <option value="KB-TK" {{ old('unit') == 'KB-TK' ? 'selected' : '' }}>KB-TK IT</option>
                            ...
                            <option value="Umum" {{ old('unit') == 'Umum' ? 'selected' : '' }}>Umum / Manajemen</option>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-hint">Angka lebih kecil tampil lebih dulu di website (misal: 1 untuk Ketua LPP).</div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="bio" name="bio" class="form-control" rows="3" placeholder="Contoh: Pengalaman lebih dari 15 tahun di bidang manajemen kurikulum Islam terpadu...">{{ old('bio') }}</textarea>
                    @error('bio') <span class="text-danger">{{ $message }}</span> @enderror
                </div>

                <div class="form-group">
                    <label class="form-label" for="image">Foto Profil (Opsional)</label>
                    <input type="file" id="image" name="image" class="form-control" accept="image/*" onchange="previewImage(this)">
                    <div class="form-hint">Format didukung: JPG, PNG, WEBP. Maks 2MB. Jika dikosongkan, inisial nama akan otomatis dijadikan avatar visual yang rapi.</div>
                ...
                <div style="margin-top: 2rem; display: flex; gap: 1rem;">
                    <button type="submit" class="btn btn-primary" style="display: inline-flex; align-items: center; gap: 0.5rem;">
                        <i data-feather="save"></i> Simpan Data SDM
                    </button>
